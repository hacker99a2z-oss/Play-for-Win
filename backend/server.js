...
// ==================== 🟢 নতুন যোগ করা কোড 🟢 ====================
// ৩.১ ম্যাচ ফিনিশ বা আর্লি এক্সিট API (Match Finish / Early Exit)
app.post('/api/match/finish', async (req, res) => {
  try {
    const { matchId, telegramId, score, timeTaken } = req.body;

    if (!matchId || !telegramId) {
      return res.status(400).json({ success: false, message: 'Match ID and Telegram ID required' });
    }

    let match = await Match.findById(matchId);
    if (!match) return res.status(404).json({ success: false, message: 'Match not found' });

    // প্লেয়ারের ডাটা খুঁজে আপডেট করা
    ...
    if (playerIndex !== -1) {
      ...
      match.players[playerIndex].finishedAt = new Date(); // ফিনিশ টাইম সেভ করা হলো
    } else {
      match.players.push({
        telegramId: String(telegramId),
        hits: Number(score) || 0,
    ...
    // সব প্লেয়ার গেম শেষ বা এক্সিট করেছে কিনা তা চেক করা
    const isFull = match.players.length >= match.mode;
    const allFinished = isFull && match.players.every(p => p.finishedAt);
    ...
    if (allFinished) {
      ...
      // স্কোর এবং সময় অনুযায়ী র‍্যাঙ্ক সাজানো (বিজয়ী নির্ধারণ)
      match.players.sort((a, b) => {
        if (b.hits !== a.hits) return b.hits - a.hits;
        return (a.timeTaken || 0) - (b.timeTaken || 0);
      });

      // মোড অনুযায়ী বিজয়ী প্রাইজ বণ্টন
      if (match.mode === 2) {
        if (match.players[0]) match.players[0].prizeUSD = 0.10;
        if (match.players[1]) match.players[1].prizeUSD = 0.00;
      } else if (match.mode === 4) {
      ...
      // বিজয়ী প্লেয়ারদের অ্যাকাউন্টে প্রাইজ যোগ করা
      for (const p of match.players) {
        if (p.prizeUSD > 0) {
          await User.findOneAndUpdate(
            { telegramId: p.telegramId },
